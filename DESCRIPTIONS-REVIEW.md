# Draft meta descriptions — FOR REVIEW BEFORE LAUNCH

20 pages on the old Weebly site had no `<meta name="description">`. The drafts
below were written from each page's actual copy (~150–160 chars, local-SEO
aware) and are applied at build time by `scripts/extract.mjs`
(`DRAFT_DESCRIPTIONS`). The 13 pages that already had descriptions keep their
originals untouched.

**To change one:** edit the text in `scripts/extract.mjs`, run
